...
CREATE DATABASE IF NOT EXISTS Metis DEFAULT CHARACTER SET utf8mb4;
USE Metis;
...
`Translater` INT DEFAULT NULL COMMENT'译者ID',
`Press` MEDIUMINT NOT NULL COMMENT'出版社编号',
`PressTime` DATE COMMENT'出版时间',
`Picture` VARCHAR(50) COMMENT'封面图片',
...
BEGIN
DECLARE thissold INT;
DECLARE amount INT;
DECLARE totalsold INT;
SET sold=(SELECT `Total` FROM `order` WHERE `OrderID`= new.orderid);
SET amount=(SELECT `Stock` FROM `book` WHERE `ISBN`=new.ISBN);
SET totalsold=(SELECT `TotalSales` FROM `book` WHERE `ISBN`=new.ISBN);
UPDATE book SET `Stock`=(amount-thissold),`TotalSales`=(totalsold+thissold) WHERE `ISBN`=new.ISBN;
END$
DELIMITER ;
/*
...
BEGIN
DECLARE thisbuyin INT;
DECLARE amount INT;
DECLARE totalthisbuyin INT;
SET thisbuyin=(SELECT `Total` FROM `order` WHERE `OrderID`= new.orderid);
SET amount=(SELECT `Stock` FROM `book` WHERE `ISBN`=new.ISBN);
SET totalthisbuyin=(SELECT `TotalSales` FROM `book` WHERE `ISBN`=new.ISBN);
IF (SELECT `Result` FROM `return` WHERE `OrderID`= new.orderid)='是' THEN
UPDATE book SET `Stock`=(amount+thisbuyin),`TotalSales`=(totalthisbuyin-thisbuyin) WHERE `ISBN`=new.ISBN;
...
BEGIN
DECLARE totalcore FLOAT(2,1);
DECLARE thisscore CHAR(1);
DECLARE total INT;
SET totalcore=(SELECT `Score` FROM `book` WHERE `ISBN`=new.ISBN);
SET thisscore=(SELECT `Score` FROM NEW);
SET total=(SELECT `TotalSales` FROM `book` WHERE `ISBN`=new.ISBN);
UPDATE `book` SET `Score`=(totalcore*(total-1)+CONVERT(thisscore,SIGNED))/total WHERE `ISBN`=new.ISBN;
END$
DELIMITER ;
/*
...
DECLARE thiscredit SMALLINT;
DECLARE totalcredit SMALLINT;
IF(SELECT `Status` FROM `order` WHERE orderid=new.orderid)='已收货' THEN
SET thiscredit=(SELECT `credit` FROM `user` WHERE `id`=new.id);
SET totalcredit=(SELECT `total` FROM `order` WHERE `orderid`=new.orderid);
UPDATE `user` SET `credit`=(thiscredit+totalcredit) WHERE `id`=new.id;
END IF;
IF(SELECT `state` FROM `user` WHERE `id`=new.id)='初级' AND (thiscredit+totalcredit)>=50 THEN
UPDATE `user` SET `state`='中级' WHERE `id`=new.id;
END IF;
IF(SELECT `state` FROM `user` WHERE `id`=new.id)='中级' AND (thiscredit+totalcredit)>=150 THEN
